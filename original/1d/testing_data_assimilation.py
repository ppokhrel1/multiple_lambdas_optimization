...
class UniformAverageBaseline(EnhancedMultiSourceBase):
    ...
    def forward(self, x: torch.Tensor, measurements: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        """Forward pass for uniform average baseline"""
    ...
    def compute_loss(
        ...
        """Compute loss for uniform average baseline"""
...
class SingleTimeScaleLagrangianOptimizer(EnhancedMultiSourceBase):
    ...
    def compute_lagrangian_weights(self) -> torch.Tensor:
        ...
        log_weights = F.log_softmax(self.lambda_weights, dim=0)
    ...
    def compute_loss(
        ...
        # Total loss
        total_loss = meas_loss + 1e-3 * physics_loss + 1e-3 * constraint_loss
...
class TwoTimeScaleLagrangianOptimizer(EnhancedMultiSourceBase):
    ...
    def compute_lagrangian_weights(self) -> torch.Tensor:
        ...
        weights = torch.exp(self.lambda_weights)
        sum_weights = weights.sum()
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, Dict[str, float]]:
        ...
        for name, model in self.models.items():
            ...
            if name == 'lagrangian_two_scale':
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
                self.optimizers[name].step()
            weights_array = meta['weights'].detach().cpu().numpy() if hasattr(meta['weights'], 'detach') else meta['weights']
    ...
    def train_epoch(self, dataloader: DataLoader) -> Dict[str, Dict[str, float]]:
        ...
        for batch in dataloader:
            ...
            for name, metrics in batch_metrics.items():
                for k, v in metrics.items():
                    if k == 'weights':
                        # Store the entire weights array
        ...
        for name, metrics in epoch_metrics.items():
            ...
            for k, v_list in metrics.items():
                if k == 'weights' and v_list:
                    # For weights, compute mean across batches while keeping the array structure
                    # v_list is a list of numpy arrays
                    if isinstance(v_list[0], np.ndarray):
                        # Stack all weight arrays and compute mean
                        stacked_weights = np.stack(v_list)
                        avg_metrics[k] = stacked_weights.mean(axis=0)  # Average across batches
                    else:
                        avg_metrics[k] = np.mean(v_list)
                elif v_list:
                    avg_metrics[k] = float(np.mean(v_list))
            result_metrics[name] = avg_metrics
