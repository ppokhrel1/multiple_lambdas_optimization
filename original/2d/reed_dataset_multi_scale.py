    ...
    def _get_stable_config(self) -> Dict:
        ...
        configs = {
            ...
            'lagrangian_two': {
                ...
                'dual_lr': 1e-5,    # Increase from 1e-4 to 1e-3
    ...
    def _create_stable_combiner(self):
        ...
        if self.combination_method == 'softmax':
            ...
            combiner = LagrangianTwoScaleCombiner(
                self.scales, self.in_channels, self.out_channels,
                primal_lr=self.method_config.get('primal_lr', 1e-4),
                dual_lr=self.method_config.get('dual_lr', 4e-5),
    ...
    for method in combination_methods:
        ...
        try:
            ...
            if test_predictions and test_targets:
                ...
                overall_test_mse = F.mse_loss(all_preds, all_targets).item()
                rmse = torch.sqrt(torch.tensor(overall_test_mse)).item()
                relative_error = torch.mean(
                    torch.abs(all_preds - all_targets) / (torch.abs(all_targets) + 1e-6)
                ).item()
            else:
                overall_test_mse = float('inf')
                rmse = float('inf')
            ...
            print(f"{'='*60}")
            print(f"Train MSE:     {avg_train_loss:.6e}")
            print(f"Overall Test MSE: {overall_test_mse:.6e}")
            print(f"Test RMSE:     {rmse:.6e}")
            print(f"Relative Error: {relative_error:.4%}")
            print(f"Best Validation Loss: {results[method]['best_val_loss']:.6e}")
            
            # Print test losses per scale
            print(f"\nTest MSE per scale:")
            for scale, loss in avg_test_losses.items():
                print(f"  Scale {scale}x{scale}: {loss:.6e}")
