...
\textcolor{red}{While Theorem~\ref{thm:unified_convergence} establishes asymptotic convergence rates of $O(1/\sqrt{k})$ and $O(1/k)$ , it is important to note that the bounding constants ($C_\theta$, $C_\lambda$, $C_c$) are not purely intrinsic to the framework. In practice, these constants are directly influenced by the initialization bias of the source weights $\lambda_0$  and the choice of learning rates. Furthermore, our theoretical proofs rely on uniform Lipschitz regularity assumptions ($L_\theta = \max_i L_{\theta,i}$). In empirical applications, sources possess heterogeneous Lipschitz constants, i.e., the global step size is constrained by the least regular source. \textcolor{blue}{In our experiments the source weights are updated on a slower time scale than the model parameters ($\eta_\lambda \ll \eta_\theta$, realizing the two-time-scale separation $\eta_\lambda = O(\eta_\theta^2)$ of Assumption~\ref{assump:auglag}), and both primal step sizes are annealed by a step-decay schedule so that the diminishing-step conditions are met. The runs therefore operate in the two-time-scale stochastic-approximation regime under which the asymptotic rates of Theorem~\ref{thm:unified_convergence} apply; the residual fluctuations visible in the convergence profiles reflect stochastic-gradient variance over the finite optimization horizon. Finally, the experiments optimize with Adam, a diagonally preconditioned variant of stochastic gradient descent; because we clip gradients (which bounds the second-moment estimate) and Adam's denominator is bounded below by $\epsilon>0$, the effective preconditioner $P_k$ satisfies $cI \preceq P_k \preceq CI$ uniformly for constants $0<c\le C<\infty$, so Theorem~\ref{thm:unified_convergence} carries over to the preconditioned iterates with the rate constants rescaled by $c$ and $C$.}}
...
\begin{itemize}
    \item Learning rates: \textcolor{blue}{$\eta_\theta = 10^{-5}$ for the model parameters and $\eta_\lambda = 10^{-7}$ for the source weights, so that the weights evolve on a slower time scale ($\eta_\lambda \ll \eta_\theta$) as required by the two-time-scale separation $\eta_\lambda = O(\eta_\theta^2)$ of Assumption~\ref{assump:auglag}; the Lagrange-multiplier (dual) ascent is at $10^{-2}$--$10^{-4}$ depending on the experiment, with weight decay $10^{-7}$. Both primal step sizes follow a step-decay schedule (halved every $150$ epochs), supplying the diminishing-step behavior under which Theorem~\ref{thm:unified_convergence} attains its asymptotic rates.}
