...
\begin{enumerate}[leftmargin=*,itemsep=0.4em]
    ...
    \item \audit{Learning-rate schedule reconciled with the code.} The manuscript
    previously described a ``decaying (step-decay) learning-rate schedule.'' The
    released code uses fixed learning rates (primal $\eta_\theta=\eta_\lambda=10^{-5}$;
    dual/multiplier ascent $10^{-2}$--$10^{-3}$; weight decay $10^{-7}$) with no
    scheduler. We corrected the stated learning rates in \S5.2 and reframed the
    text (\S4.5, \S5.2): the runs operate in the constant-step
    stochastic-approximation regime and converge to a neighborhood of the
    optimum, which the asymptotic $O(1/\sqrt{k})$/$O(1/k)$ rates of Theorem~2
    idealize---consistent with the residual fluctuations visible in the
    convergence figures. The ``step decay'' claims have been removed.
\end{enumerate}
